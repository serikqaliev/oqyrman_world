...
/// Данный код представляет собой реализацию домашней страницы приложения для чтения книг.

/// Первые строки импортируют необходимые библиотеки и классы.

/// Класс `HomePage` является StatelessWidget, и он определяет верхний уровень виджета,
/// который строит домашнюю страницу. Внутри этого класса определяется AppBar и основное содержимое страницы.

/// Класс `_HomePageBody` является StatefulWidget и представляет собой тело домашней страницы.
/// Он содержит категории книг, которые можно выбирать, и списки книг.

/// Класс `_VerticalList` является StatefulWidget и содержит вертикальный список всех книг.

/// Для управления состоянием на домашней странице используется BLoC-архитектура.
/// Виджеты подписываются на изменения состояния и перестраиваются только при необходимости.

/// Этот код написан на языке программирования Dart для использования с фреймворком Flutter.
/// Он реализует дизайн приложения для чтения книг и обеспечивает удобный пользовательский интерфейс
/// для выбора книг, отображения информации о них и добавления в список избранных книг.
